...
#define GET_KEY_LENGTH(x) (((x) >> 24) & 0xFF)
#define GET_HASH_DATA(start, offset) \
		(HashData*)((char*)start + ((offset) & 0xFFFFFF))

static HashData* hashmapFind(Hashmap* map, const char* key)
{
	if (map == NULL || key == NULL)
	{
		fprintf(stderr, "hashmapFind : argument is null\n");
		return NULL;
	}
...
int hashmapPut(Hashmap* map, const char* key, const char* value)
{
	...
	else
	{
		...
		memcpy(hashData[map->count].key, key, keyLen + 1); // null문자 포함
		memcpy(hashData[map->count].value, value, valLen + 1); // null문자 포함
...
const char* hashmapGet(Hashmap* map, const char* key)
{
	...
	if (data == NULL)
	{
		fprintf(stderr, "hashmapGet : hashmapFind error\n");
		return NULL;
	}
...
int main()
{
	char* key[6] = { "apple", "banana", "cherry", "orange", "dragonFruit", "fruit" };
	char* value[6] = { "사과", "바나나", "체리", "오렌지", "용과", "과일" };
