...
// 임계점이 넘어가면 2배로 리해싱되도록 코드를 수정해보자
...
static int expandIfNecessary(Hashmap* map)
{
	if (map == NULL)
	{
		fprintf(stderr, "expandIfNecessary : argument is null\n");
		return -1;
	}
	...
	size_t newBucketSize = map->bucketSize << 1;
	Node** newBuckets = calloc(newBucketSize, sizeof(Node*));
	if (newBuckets == NULL)
	{
		perror("expandIfNecessary");
		return -1;
	}
